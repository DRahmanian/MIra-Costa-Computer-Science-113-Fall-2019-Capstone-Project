...
import edu.miracosta.cs112.mavericks.Backend.Seafoods;
...
public class SeafoodScene extends Scene {
    private Seafoods currentSeafood;
    ...
    private TextField fishTF;

    public SeafoodScene(Scene prevScene) {
        ...
        fishTF = new TextField();

        Button addButton = new Button("Add Seafood");
    ...
    private void addDish(){
        ...
        if(!carbsTF.getText().equals(""))
            ...
            carbs = 0.0;
        String fish = fishTF.getText();
        currentSeafood = new Seafoods(name, description, fats, calories, carbs, fish);

        if(currentSeafood != null) {
            Controller controller = Controller.getInstance();
            controller.getAllDishList().add(currentSeafood);
        } else {
            return;
